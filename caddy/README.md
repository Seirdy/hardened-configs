...
Note: TLS/HTTPS compression may add attack surface, especially gzip, and especially compression before encryption. Trades some security for performance. See [CRIME](https://wikipedia.org/wiki/CRIME) and [BREACH](https://en.wikipedia.org/wiki/BREACH).
...
- `-Server` Removes the `Server` header that tells clients what web server is being used and the version.
- `X-Robots-Tag` set to `none` tells (respecting) search engine spiders and indexers to not index or crawl our website. Remove if you want the website to be crawled.
- `X-Content-Type-Options` set to `nosniff` prevents the browser from MIME and content sniffing.

`Content-Security-Policy` is a pretty important header as it defines how browsers handle certain resources such as sandboxing them, restricting them, and access control. A strong, strict CSP prevents many kinds of attacks.
...
<br>
`Content-Security-Policy "default-src 'none'; script-src 'self'; object-src 'self'; style-src 'self'; img-src 'self'; media-src 'self'; frame-src 'self'; font-src 'self'; connect-src 'self'; manifest-src 'self'; block-all-mixed-content upgrade-insecure-requests;"`
...
- `form-action 'none'` forbids all HTML forms from anywhere. This is implied by `default-src`.
- `frame-ancestors 'none'` is the new equivalent to `X-Frame-Options "DENY"`. Forbids embedding `<frame> <iframe> <object> <embed> <applet>`. This is implied by `default-src`.
- `child-src 'none'` is the CSP v3 replacement for `frame-src`. Defines which resources are allowed to be embedded using `<frame>` and `<iframe>`. This is implied by `default-src`.
- `upgrade-insecure-requests` automatically upgrades all `http://` resources and links on a website to `https://`
- `block-all-mixed-content` is a deprecated policy that forbids loading content from HTTP sources on an HTTPS website.

Note: `X-Content-Security-Policy` and `X-WebKit-CSP` are the same. They are just for browser compatibility such as WebKit.
